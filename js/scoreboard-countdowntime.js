...
(function($) {
	$.fn.npyScorecount = function(options) { // 플러그인 명세 .npyScorecount([options]);
		...
		function initScoreboard(length) { // 스코어보드 생성
			...
			obj.find('.npy-scdigit').css({ position: 'relative', width: $digitWidth, height: $digitHeight, backgroundImage: 'url(https://www.centarahotelsresorts.com/countdowntime/sc_digits.png)', backgroundSize: '1100%', backgroundPositionX: ($responsive) ? '100%' : $digitWidth*(-10), backgroundPositionY: 0, backgroundRepeat: 'no-repeat', display: 'inline-block', verticalAlign: 'middle' });
			obj.find('.npy-scdigit > div').css({ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', backgroundImage: 'url(https://www.centarahotelsresorts.com/countdowntime/sc_digits.png)', backgroundSize: '1100%', backgroundPositionX: 0, backgroundRepeat: 'no-repeat', display: 'inline-block' });
